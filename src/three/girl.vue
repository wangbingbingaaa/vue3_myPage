...
onMounted(() => {
    ...
    // let meshbg = createPanorama(BG); //球体全景
    let geometry = new THREE.SphereGeometry(500, 100, 100);
    let material = new THREE.MeshBasicMaterial({
        map: new THREE.TextureLoader().load('./obj/pImg1.png'), //导入图片
        color: 0xffffff,
        side: THREE.BackSide,
    });
...
const addAniControl = (Anis: any[]) => {
    ...
    const playAni = function (ani: { name: string; play: () => void; setLoop: (arg0: any) => void; reset: () => void; crossFadeTo: (arg0: any, arg1: number, arg2: boolean) => void; _clip: { duration: number } }) {
        if (ani.name == 'idle') {
            ani.play();
            return;
        } else {
    ...
    if (!document.querySelector('#anisPanelStyle')) {
        ...
        anisPanelStyle.innerText += '#anisPanel{position:fixed;display:flex;flex-wrap:wrap;justify-content:center; width:100%;bottom:15%;left:50%;transform:translate(-50%,0); color:#515151;cursor:pointer;}\
                               #anisPanel li{line-height:30px;text-align:center;border:1px solid #fff; font-size:14px;margin:10px;min-width:100px;height:50px;line-height:50px;overflow:hidden; border-radius:50px;background:rgba(250,218,193,0.8);}#anisPanel li.on,#anisPanel li:hover{color:#fff;background:rgba(73,73,73,0.8);}';
